...
router.post('/:shipmentId/location', authenticate, authorize('transport'), async (req, res, next) => {
  try {
    ...
    // Track demo flag - location is clearly labelled
    const demoFlag = isDemo !== false && (isDemo === true || /demo|sample|simulat/i.test(location || ''));
...
router.post('/:shipmentId/proof', authenticate, authorize('transport'), async (req, res, next) => {
  try {
