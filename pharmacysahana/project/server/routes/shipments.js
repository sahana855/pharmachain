...
async function validateShipmentPreconditions(medicines, toUser) {
  const now = new Date();
  const issues = [];
  for (const med of medicines) {
    if (med.status !== 'active') {
      issues.push(`${med.name} status is ${med.status}`);
    }
    if (med.expiryDate && new Date(med.expiryDate) < now) {
      issues.push(`${med.name} has expired`);
    }
  }
  if (issues.length > 0) {
    throw Object.assign(new Error(`Cannot ship: ${issues.join(', ')}`), { status: 400 });
  }
  if (toUser.verificationStatus !== 'verified') {
    throw Object.assign(new Error(`Destination ${toUser.name} (${toUser.role}) is not approved`), { status: 403 });
  }
}
...
router.post('/', authenticate, authorize('manufacturer', 'dealer'), requireVerified, async (req, res, next) => {
  try {
    ...
    const medicines = await validateShipmentItems(items, req.user);
    await validateShipmentPreconditions(medicines, toUser);
...
router.patch('/:id/assign', authenticate, authorize('manufacturer', 'dealer', 'admin'), requireVerified, assignTransportHandler);
router.post('/:id/assign', authenticate, authorize('manufacturer', 'dealer', 'admin'), requireVerified, assignTransportHandler);
...
const updateStatusHandler = async (req, res, next) => {
  try {
    ...
    if (status === 'DISPATCHED') {
      const medicines = await Medicine.find({ _id: { $in: shipment.items.map((it) => it.medicineId).filter(Boolean) } });
      const toUser = await User.findById(shipment.toId);
      await validateShipmentPreconditions(medicines, toUser);
    }
...
router.patch('/:id/status', authenticate, authorize('manufacturer', 'dealer', 'transport', 'pharmacy', 'admin'), requireVerified, updateStatusHandler);
router.post('/:id/status', authenticate, authorize('manufacturer', 'dealer', 'transport', 'pharmacy', 'admin'), requireVerified, updateStatusHandler);

// POST /api/shipments/:id/accept - destination dealer or pharmacy accepts delivery
router.post('/:id/accept', authenticate, authorize('dealer', 'pharmacy'), requireVerified, async (req, res, next) => {
  try {
    const shipment = await Shipment.findById(req.params.id);
    if (!shipment) return res.status(404).json({ success: false, error: 'Shipment not found' });
