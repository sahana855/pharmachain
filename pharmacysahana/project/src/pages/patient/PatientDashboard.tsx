...
import { AlertTriangle, Clock, ShoppingCart, Activity, Truck, RefreshCw, Waypoints, CheckCircle, ScanLine } from 'lucide-react';
...
export default function PatientDashboard() {
  ...
  const [stats, setStats] = useState({ activeShipments: 0, deliveredShipments: 0, delayedShipments: 0, totalTracked: 0 });
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setStats({
        ...
        deliveredShipments: myShipments.filter((s: any) => s.status === 'DELIVERED' || s.status === 'DELIVERED_TO_PHARMACY').length,
        delayedShipments: myShipments.filter((s: any) => s.status === 'DELAYED').length,
        totalTracked: myShipments.length,
      });
    ...
    <div className="space-y-6">
        ...
        <Card>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{stats.delayedShipments}</p>
            </div>
          </div>
        </Card>
        <Card>
      ...
      <Card title="Quick Actions" subtitle="Access your health management tools">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <button onClick={() => navigate('/patient/authenticity')} className="p-4 bg-green-50 rounded-lg text-center hover:bg-green-100 transition"><ScanLine className="mx-auto mb-2 text-green-600" size={24} /><p className="text-sm font-medium text-gray-700">Verify Medicine</p></button>
