 ...
 */
router.post('/create', authenticate, authorize('manufacturer', 'dealer'), async (req, res, next) => {
  try {
    const {
      shipmentId,
      source,
    ...
    const box = await TransportBox.create({
      ...
      isDemo: true, // box tracking uses demo location data until GPS is wired
    });
 ...
 */
router.post('/:boxId/scan', authenticate, authorize(...SCAN_ROLES), async (req, res, next) => {
  try {
    const boxId = normalizeBoxId(req.params.boxId);
    if (!boxId) return res.status(400).json({ success: false, error: 'Invalid box QR format' });
 ...
 */
router.put('/:boxId/status', authenticate, async (req, res, next) => {
  try {
 ...
 */
router.post('/:boxId/location', authenticate, authorize('transport'), async (req, res, next) => {
  try {
    const { location, latitude, longitude, remarks } = req.body;
    ...
    // Demo flag - clearly labelled
    const demoFlag = /demo|sample|simulat/i.test(location || '') ? true : box.isDemo;
