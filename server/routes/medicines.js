...
router.post('/register', authenticate, authorize('manufacturer'), async (req, res, next) => {
  try {
    const {
      name,
      batchNumber,
    ...
    const manufacturer = await req.user;

    const medicine = await Medicine.create({
      name,
      batchNumber,
      manufacturerId: req.user.id,
      manufacturerName: req.user.name || manufacturer.name,
      quantity,
      price,
      manufacturingDate,
      expiryDate,
...
router.patch('/:id/status', authenticate, authorize('manufacturer', 'admin'), async (req, res, next) => {
  try {
