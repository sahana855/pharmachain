...
import { shipmentApi, medicineApi } from '../../lib/api';
import { useLiveSync } from '../../lib/events';
import { Package, AlertTriangle, RefreshCw, Waypoints, Truck, CheckCircle } from 'lucide-react';
...
export default function PharmacyDashboard() {
  ...
  const [stats, setStats] = useState({ totalReceived: 0, pendingVerification: 0, expiringSoon: 0, activeShipments: 0, deliveredCount: 0, inTransitCount: 0 });
  ...
  const fetchData = useCallback(async () => {
    try {
      const [shipData, medData] = await Promise.all([
        shipmentApi.list(),
        medicineApi.list(),
      ]);
      ...
      const deliveredShipments = myShipments.filter((s: any) => s.status === 'DELIVERED_TO_PHARMACY' || s.status === 'DELIVERED');
      const totalReceivedItems = deliveredShipments.reduce((sum: number, s: any) => sum + (s.items?.length || 0), 0);

      setStats({
        totalReceived: totalReceivedItems,
        pendingVerification: myShipments.filter((s: any) => s.status === 'IN_TRANSIT' || s.status === 'DISPATCHED' || s.status === 'OUT_FOR_DELIVERY').length,
        expiringSoon: medicines.filter((m: any) => m.expiryDate && new Date(m.expiryDate) <= threeMonths && new Date(m.expiryDate) > now).length,
        activeShipments: myShipments.filter((s: any) => s.status === 'DISPATCHED' || s.status === 'IN_TRANSIT' || s.status === 'OUT_FOR_DELIVERY').length,
        deliveredCount: deliveredShipments.length,
        inTransitCount: myShipments.filter((s: any) => s.status === 'IN_TRANSIT').length,
      });
          ...
          {recentShipments.map((shipment) => (
            <div key={shipment._id || shipment.id} className="p-3 bg-gray-50 rounded-xl border border-gray-100 hover:border-primary-200 transition cursor-pointer" onClick={() => navigate(`/track/${shipment.shipmentQrId}`)}>
              <div className="flex items-center justify-between mb-1">
                ...
                <Badge variant={
                  shipment.status === 'DELIVERED' || shipment.status === 'DELIVERED_TO_PHARMACY' ? 'success' :
