  ...
  const fetchShipments = async () => {
    try {
      ...
      const incoming = all.filter((s: any) =>
        s.toId === user?.id &&
        ['ASSIGNED_TO_DEALER', 'CREATED'].includes(s.status)
      );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">Shipments from manufacturers awaiting your acceptance</p>
      </div>

      {success && <div className="p-4 bg-green-50 border border-green-200 text-green-700 rounded-lg text-sm">{success}</div>}

      <Card title="Pending Acceptance" subtitle={`${incomingShipments.length} shipments pending`} icon={<ShoppingCart />}>
              ...
              <div key={shipment._id || shipment.id} className="p-4 border border-gray-200 rounded-lg">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">From: {shipment.fromName}</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <p className="text-xs text-gray-400">{new Date(shipment.createdAt).toLocaleString()}</p>
                  <Button size="sm" variant="success" onClick={() => handleAccept(shipment._id || shipment.id)}>
