...
export default function ReturnsPharmacy() {
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div><h1 className="text-2xl font-bold text-gray-900">Medicine Returns</h1><p className="text-gray-500 mt-1">Return damaged or unwanted medicines</p></div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="New Return Request">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Reason</label>
            <textarea value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" rows={3} required /></div>
          <Button type="submit" className="w-full"><AlertCircle size={16} /> Submit Return</Button>
        </form>
      </Modal>
    </div>
  );
